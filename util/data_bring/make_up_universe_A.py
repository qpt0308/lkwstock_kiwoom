...
def crawler(code, page):
    ...
    # column명을 가공
    header_data = ["시장구분","종목코드"]
    for item in table_html.select('thead th'):
        if item.get_text().strip() != "N" and item.get_text().strip() != "토론실" :
            item.get_text().replace('(','_').replace(')','_')
            header_data.append(item.get_text().strip())
    ...
    for item in table_html.find_all(lambda x:
                                    ...
                                    (x.name == 'td' and
                                     'number' in x.get('class', []) or ('center' in x.get('class',[])))
                                    ):
        if item.get("class")[0] == 'tltle':
            temp_data.append(code_d)
            temp_data.append(str(item.get("href")).strip().split("=")[-6:][1])
            temp_data.append(item.get_text().strip())
        elif item.get("class")[0] == 'center':
            inner_data.append(temp_data)
            temp_data =[]
        else:
            temp_data.append(item.get_text().strip())


    #logger.debug("구분 : {} , page : {} ".format(code_d,page))
    print("구분 : {} , page : {} ".format(code_d, page))
    ...
    df = pd.DataFrame(data=inner_data, columns=header_data)
    #logger.debug(df)
    return df
...
def get_universe():
    ...
    # 사용할 column들 설정
    cols = ['거래량','등락률','현재가','시가','고가','저가','거래대금', '매출액', '매출액증가율', 'ROE', 'PER']

    # column들을 숫자타입으로 변환(Naver Finance를 크롤링해온 데이터는 str 형태)
    df[cols] = df[cols].astype(float)

    # 유니버스 구성 조건 (1)~(4)를 만족하는 데이터 가져오기
    df = df[(((df['고가'] - df ['저가']) / df['저가'] > 0.05) & (df['등락률'] >= -5) & (df['거래량'] > 0) & (df['거래대금'] > 0) & (~df.종목명.str.contains("지주")) & (~df.종목명.str.contains("홀딩스")))]
    ...
    return df[['종목코드','종목명','등락률','현재가','시가','저가','고가']]
