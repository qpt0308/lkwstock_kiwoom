...
class Kiwoom(QAxWidget):
    ...
    def __del__(self):
        send_message(f"{datetime.now()} kiwoom 종료되었습니다.!!")
        logger.debug(f"{datetime.now()} kiwoom 종료되었습니다.!!")
    ...
    def working_exs(self):
        start_time = time.time()
        while not self.requs_ok:
            QTest.qWait(self.working_exs_time)
    ...
    def comm_connect(self):
        try:
            ...
            while self.loop_bool :
                QTest.qWait(self.loop_wait_time)
            self.loop_bool = True
            self.requs_ok = True
        except Exception as e:
            print(traceback.format_exc())
    ...
    def get_account_number(self, tag="ACCNO"):
        try:
            ...
            logger.debug(f"{account_number},{account_list}")
            print(f"{account_number},{account_list}")
            self.requs_ok = True
    ...
    def get_price_data(self, code):
        try:
            ...
            df = pandas.DataFrame(ohlcv, columns=['open', 'high', 'low', 'close', 'volume'], index=ohlcv['date'])
    ...
    def _on_receive_tr_data(self, screen_no, rqname, trcode, record_name, next, unused1, unused2, unused3, unused4):
        try:
            ...
            #logger.debug("[Kiwoom] _on_receive_tr_data is called {} / {} / {}".format(screen_no, rqname, trcode))
            ...
            if rqname == "opt10081_req":
                ...
                logger.debug("실제예수금 : {}".format(self.tr_data))
                #print("실제예수금 : {}".format(self.tr_data))
                self.loop_bool = False
    ...
    def _on_receive_msg(self, screen_no, rqname, trcode, msg):
        try:
            logger.debug("[Kiwoom] _on_receive_msg is called {} / {} / {} / {}".format(screen_no, rqname, trcode, msg))
    ...
    def _on_chejan_slot(self, s_gubun, n_item_cnt, s_fid_list):
            try:
                logger.debug("[Kiwoom] _on_chejan_slot is called {} / {} / {}".format(s_gubun, n_item_cnt, s_fid_list))
                ...
                if int(s_gubun) == 0:
                    logger.debug(f"*주문/접수/체결 출력(slef.order){self.order}")
                    print(self.order)
                elif int(s_gubun) == 1:
                    pass
                    logger.debug(f"잔고 출력(self.balance){self.balance}")
                    print(self.balance)
            except Exception as e:
                print(traceback.format_exc())
    ...
    def _on_receive_real_data(self, s_code, real_type, real_data):
        try:
            if real_type == "장시작시간":
                pass
                #make_gubun = self.dynamicCall("GetCommRealData(QString, int)", s_code, get_fid("장운영구분"))
                #deal_time = self.dynamicCall("GetCommRealData(QString, int)", s_code, get_fid("체결시간"))
                #make_start_residual = self.dynamicCall("GetCommRealData(QString, int)", s_code, get_fid("장시작예상잔여시간"))
                #self.make_real_time.update({'장운영구분':make_gubun,'체결시간':deal_time,'장시작예상잔여시간':make_start_residual})
                ...
                fa_st = self.dynamicCall("GetCommRealData(QString, int)", s_code, get_fid('체결강도'))
                fa_st = float(fa_st)

                rotation = self.dynamicCall("GetCommRealData(QString, int)", s_code, get_fid('거래회전율'))
                rotation = float(rotation)

                make_gubun = self.dynamicCall("GetCommRealData(QString, int)", s_code, get_fid('장구분'))
                make_gubun = float(make_gubun)

                # logger.debug(s_code, signed_at, close, high, open, low, top_priority_ask, top_priority_bid, accum_volume)
                ...
                self.universe_realtime_transaction_info[s_code].update({
                    ...
                    "저가": low,
                    "현재가": close,
                    "최우선매도호가": top_priority_ask,
                    "최우선매수호가": top_priority_bid,
                    "누적거래량": accum_volume,
                    "체결강도" : fa_st,
                    "거래회전율" : rotation,
                    "장구분" : make_gubun,
